...
class Dashboard:
    ...
    def render(self):
        ...
        with col1:
            self.metric_card("Model Accuracy", f"{self.get_latest_metric('accuracy'):.2%}",
                             "How well the model performs on test data")

        with col2:
            self.metric_card("Training Loss", f"{self.get_latest_metric('training_loss'):.4f}",
                             "Error rate during training (lower is better)")

        with col3:
            self.metric_card("Validation Loss", f"{self.get_latest_metric('validation_loss'):.4f}",
